...
import os
from os import path
from Scrapper.ScrapData import GetGenSeqGB
filePath = "Fichiers externes"

def MainFunction(gene): # Fonction principale -> Nom du gène en entrée ; Request : booléen pour éviter de renvoyer des requête à chaque fois
    ...
    if path.exists(filePath + "/Gène " + gene) == False:
        os.mkdir(filePath +  "/Gène " + gene)
...
def Etape_C(gene):
    ...
    if gene == "S":
        name = "spike"
    SeqIO.write(listSeqRecord, filePath + "/Gène " + gene + "/" + name + ".fasta", "fasta") # On écris les SeqRecord dans le fichier spike.fasta au format fasta
...
def Etape_E(gene):
    ...
    ensSeq = SeqIO.parse(filePath + "/Gène " + gene + "/" + "aln-" + alnFileName + ".fasta", "fasta") # On obtient les séquences alignés
    ...
    file = open(filePath + "/Gène " + gene + "/" + "resultatComparaison_gene" + gene + ".txt", "w")
    file.write(result)
    file.close()
...
def Etape_D(gene):
    ...
    from Bio.Align.Applications import MafftCommandline
    commande = MafftCommandline(input= filePath + "/Gène " + gene + "/" + fileName + ".fasta")
    myStdout, myStderr = commande()
    with open(filePath + "/Gène " + gene + "/" + "aln-" + fileName + ".fasta", 'w') as w:
        w.write(myStdout)
...
def Etape_I(gene):
    ...
    for x in range(len(newList[8])):
        if(newList[8][x] == gene.lower()):
            listID.append(newList[9][x])

    GetGenSeqGB(gene, listID) 
