...
def Etape_E():
    ensSeq = SeqIO.parse("aln-spike.fasta", "fasta") # On obtient les séquences alignés
    seqList = list() #
    seqLen = 0
    ...
    result = result + "\n\n" + "Conservation CS : " + str(conservBatNuc / seqLen) + "\n\n" + "Conservation Pangolin : " + str(conservPangNuc / seqLen)

    file = open("resultatComparaison_geneS.txt", "w")
    file.write(result)
    file.close()
