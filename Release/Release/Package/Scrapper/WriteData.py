...
from Scrapper.ManualAlign import GetSeq
import os
from os import path
...
def WriteData(rule, gene): # fonction d'entrée ; Si rule = True on créé tous les fichiers ; Si non, on se refère au gène indiqué
    finalList = list()
    GetData() # Télécharge et créé seq_covid.gb
    global result_Etape_B, genList
    result_Etape_B = Etapes.Etape_B()
    finalList.append(result_Etape_B)

    if rule == 1: 
        fullGeneList = Etapes.GetGenList(result_Etape_B[8])
        genList = list()
        genList = fullGeneList
        finalList.append(fullGeneList)
        finalList.append(Main.CreateRepertoryAndFile(fullGeneList))
    else:
        fullGeneList = Etapes.GetGenList(result_Etape_B[8])
        finalList.append(fullGeneList)
        finalList.append(Main.CreateOneRepAndFile(gene))

    return finalList

class Main():

    def CreateRepertoryAndFile(genList): # Fonction de création des répertoire si la règle est définie sur True
        data = list()
        for gene in genList:
...
class Etapes():
    ...
    def Etape_C(gene):
        ...
        if len(listSeqRecord) == 0: # Si la liste de seqRecord à une longueur nulle
            MakeLogs("Une erreur est survenue lors de l'écriture du fichier fasta concernant le gène : " + gene) # On inscrit l'erreur dans les logs
    ...
    def Etape_D(gene):
        ...
        try: # On fait un try except pour attraper une exception si le fasta d'un gène n'existe pas
            ...
            MakeLogs("Le gène " + gene + " n'est pas alignable car le fichier n'existe pas")

        ManualAlign.WriteSeq(gene)
    ...
    def Etape_E(gene):
        ...
        try: # # On fait un try except pour attraper une exception si le fasta d'un gène n'existe pas
            ...
            MakeLogs("Le fichier d'alignement n'est pas disponible pour le gène " + gene)
            return
...
class ManualAlign():
    def WriteSeq(gene):
        ...
        ensSeq = SeqIO.parse(repPath + "/Protéine " + gene + "/" + gene + ".gb", "genbank") #Parse des trois séquences, "r")
        seqList = list()
        for seq in ensSeq:
            seqList.append(seq.seq)

        result = GetSeq(seqList[0], seqList[1])
        newResult = list()
        newResult.append(result[0])
        newResult.append(result[1])
        result = GetSeq(seqList[1], seqList[2])
        newResult.append(result[1])
        
        print(len(newResult))

        listSeqRecord = list()
        listHost = ("Humain", "Chauve-Souris", "Pangolin")

        for a in range(0, 3):
            listSeqRecord.append(SeqRecord(Seq(newResult[a]), id=listHost[a]))

        SeqIO.write(listSeqRecord, repPath + "/Protéine " + gene + "/" + "manual-align" + gene + ".fasta", "fasta") # On écris les SeqRecord dans le fichier spike.fasta au format fasta
