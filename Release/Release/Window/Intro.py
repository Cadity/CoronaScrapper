...
def DisplayIntro():
    ...
    title = tk.Label(intro, text="Corona Scrapper", font=("Arial", 25)).place(relx = 0.98,rely = 0.05,anchor ='ne')

    #Définition du MenuBoutton pour la séléction de Preset
    startButton = tk.Button(intro, text="Débuter le Processus Automatique", relief="raised", command=lambda:[activateProcessus(), intro.destroy()]).place(relx = 0.3,rely = 0.30, anchor ='center')
    creditButton = tk.Button(intro, text="Crédit", relief="raised", command=credit).place(relx = 0.13,rely = 0.42, anchor ='center')
    settingsButton = tk.Button(intro, text="Paramètres", relief="raised", command=lambda:[displaySettings(), intro.destroy()]).place(relx = 0.16,rely = 0.54, anchor ='center')
...
def activateProcessus():
    global Selected
    Selected = "activatingProcess"
...
def infoVersion():
    msgBox = messagebox.showinfo(title="Version du Logiciel", message="CoronaWrapper - Release 5.4")
